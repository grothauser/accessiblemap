...
function setManualLocation() {
	...
	$.each(candidatesForManualSearch, function(index, candidate) {
		...
		if (candidate.osm_id == wayId) {
			...
			searchOverpassForLocationCoords(lat, lon, 'way["highway"]').done(
					function(matchingSegment) {

						getWayInfoOverpass(locatedWayId).done(
								function(wayData, nodeData) {
									...
									var distSeg = new distSegmentEntry(startNode.x, startNode.y, endNode.x, endNode.y, startNode.x,
											startNode.y, dist, wayData[0].id, wayVec);
...
function getPOISHTML(side) {
	...
	$.each(streetViewContent, function(index, poi) {
		...
		if ((side == 'left') && ($.inArray(poi, alreadyPrintedPoisLeft) == -1)) {
			...
			htmlAround = htmlAround.concat('<input type="radio" data-mini="true" data-inline="true" class="radioelem"' + 'name="'
					+ radioname + '" id="' + poi.lat + "," + poi.lon + '" value="' + poi.name.concat(" " + name) + ' "  />'
					+ '<label for="' + poi.lat + "," + poi.lon + '" class="poi-label"> ' + poi.name + " " + name + " <br> in " + poi.distance
					+ " Meter auf " + poi.clock + " Uhr </label>");
		...
		if (wayVec.nodes.length > 1) {
			...
			$.each(wayVec.nodes, function(i, node) {
				...
				if (i < (wayVec.nodes.length - 1)) {
					...
					nearestSegment = new distSegmentEntry(node.x, node.y, nextNode.x, nextNode.y, lat, lon, distToSegmentResult,
							wayVec.wayId, wayVec);
...
function testOverlap(wayA, wayB) {
	...
	for (var i = 0; i < wayA.nodes.length - 1; i++) {
		...
		for (var k = 0; k < wayB.nodes.length - 1; k++) {
			...
			var warning = getOverlaps(line1Start.x, line1Start.y, line1End.x, line1End.y, line2Start.x, line2Start.y, line2End.x,
					line2End.y);
