...
function findIntersections(wayVectors,lat,lon) {
	...
	$.each(wayVectors, function(i, element){
		if(element.wayId!==locatedWay.wayId){
			...
			if (isec != -1) {
				var intersectionEntry = new intersection(isec.x, isec.y, element.tags, locatedWay.way.tags, element.wayId, locatedWay.wayId );
...
function testOverlap(wayA, wayB){
	...
	for(var i=0; i<wayA.nodes.length-1; i++){
		for(var k=0;k<wayB.nodes.length-1; k++){
			...
			if(warning!==null){
				warnings.push(new warn(warning.x, warning.y, wayA.tags, wayB.tags,wayA.wayId,wayB.wayId));
			}
		}
	}
	return warnings;
